...
path_volume = os.path.join("/Volumes", catalog, read_schema, f"{inst_name}_bronze_file_volume")
...
pdp.dataio.write_data_to_delta_table(df_course, f"{catalog}.{write_schema}.course_dataset_validated", spark_session=spark_session)

# COMMAND ----------

pdp.dataio.write_data_to_delta_table(df_cohort, f"{catalog}.{write_schema}.cohort_dataset_validated", spark_session=spark_session)
...
df_course = pdp.schemas.RawPDPCourseDataSchema(
    pdp.dataio.read_data_from_delta_table(f"{catalog}.{write_schema}.course_dataset_validated", spark_session=spark_session)
)
df_course.shape
...
df_cohort = pdp.schemas.RawCohortDataSchema(
    pdp.dataio.read_data_from_delta_table(f"{catalog}.{write_schema}.cohort_dataset_validated", spark_session=spark_session)
)
df_cohort.shape
...
_ = msno.matrix(
    df_course.sort_values(by=["academic_year", "enrolled_at_other_institution_s"], ignore_index=True),
    sparkline=False,
    labels=True,
)
...
print(f"num rows missing course id: {num_missing_course_id} ({pct_missing_course_id:.1f}%)")
...
(
    sb.histplot(
        (
            df.assign(has_pre_cohort_courses=lambda df: df["cohort"].gt(df["academic_year"]))
...
df_pre_cohort = (
    df.loc[df["cohort"].gt(df["academic_year"]), :]
    .assign(
...
print(df_cohort["enrollment_type"].value_counts(normalize=True, dropna=False), end="\n\n")
print(df_cohort["enrollment_intensity_first_term"].value_counts(normalize=True, dropna=False), end="\n\n")
print(df_cohort["credential_type_sought_year_1"].value_counts(normalize=True, dropna=False), end="\n\n")
...
first_ayear, last_ayear = df_course["academic_year"].min(), df_course["academic_year"].max()
print(f"{num_ayears} academic years ({first_ayear} through {last_ayear})")

# COMMAND ----------

num_courses = df_course["course_prefix"].str.cat(df_course["course_number"], sep=" ").nunique()
...
df_assoc_course = pdp.eda.compute_pairwise_associations(
    df_course,
    exclude_cols=["student_guid", "institution_id", "student_age", "gender", "race", "ethnicity"],
)
df_assoc_course
