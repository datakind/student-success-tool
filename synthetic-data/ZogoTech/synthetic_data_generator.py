...
columns = [
    "StuNum", "First_Enrollment_Date", "FirstGenFlag", "AgeGroup", 
    "Gender", "Race", "Ethnicity", "IncarceratedFlag", 
    "DualEnrollFlag", "ABEFlag", "Zip", "DisabilityFlag", 
    "HighSchoolStatus", "HighSchoolGradDate", "HighSchoolGPA", 
    "StudentAthlete", "EarnedAward", "AwardsEarned"
]

# Define choices for each column
stu_num_range = range(10000, 99999)  # Unique student numbers
first_gen_choices = ['Y', 'N', 'U']
age_group_choices = ['Under 18', '18-19', '20-24', '25-29', '30-39', '40-49', '50-64', '65+']
gender_choices = ['Female', 'Male', 'Not Specified']
race_choices = ['Non-Hispanic', 'Hispanic']
ethnicity_choices = [
    'Two or more races', 'Caucasian/White', 'Unknown', 
...
high_school_status_choices = ['TraditionalHighSchoolDiploma', 'No High School Record', 'GED']
...
for _ in range(num_records):
    ...
    high_school_grad_date = random.choice([random_date(datetime(1990, 1, 1), datetime.now()), None])
    high_school_gpa = random.choice([None] * 80 + [round(random.uniform(0, 4), 2) for _ in range(20)])  # 80% nulls
    student_athlete = random.choice(['Y', 'N'])
    earned_award = random.choice([None] * 80 + ['Y' for _ in range(20)])  # 80% nulls
    awards_earned = random.choice([None] * 80 + [random.randint(1, 10) for _ in range(20)])  # 80% nulls

    data.append([
        stu_num, first_enrollment_date, first_gen_flag, age_group, gender, 
        race, ethnicity, incarcerated_flag, dual_enroll_flag, abe_flag, 
        zip_code, disability_flag, high_school_status, high_school_grad_date, 
        high_school_gpa, student_athlete, earned_award, awards_earned
    ])
...
# Define choices for each column
semesters = ['2018FA', '2018SP', '2019FA', '2019SP', '2020FA', '2020SP', '2021FA', '2021SP']
enrollment_types = ['PT', 'FT']
degree_types = [
    'Associate of Science', 'Associate of Applied Science', 
    'Associate of Arts', 'Associate of Business', 
    'Associate of General Studies', 'Certificate', 
    'Non-Academic'
]
pell_recipient_choices = [None, 'Y']
majors = [
    'Custodian Studies Certificate', 'Construction Technician', 
    'Agriculture Associate of Arts', 'Business Administration', 
    'Nursing', 'Computer Science', 'Graphic Design'
]
department = [None]  # All NULL

columns = [
    "Student_ID", "Semester", "Enrollment_Type", "Degree_Type_Pursued", 
    "Intent_To_Transfer_Flag", "Pell_Recipient", "Major", "Department", 
    "Cumulative_GPA", "Semester_GPA", "Number_Of_Courses_Enrolled", 
    "Number_Of_Credits_Attempted", "Number_Of_Credits_Failed", 
    "Number_Of_Credits_Earned", "Course_Pass_Rate", "Online_Course_Rate"
]
...
def generate_valid_enrollment_data():
    ...
    return [
        number_of_courses, number_of_credits_attempted, number_of_credits_failed, 
        number_of_credits_earned, cumulative_gpa, semester_gpa, 
        course_pass_rate, online_course_rate
    ]
...
for _ in range(num_records):
    ...
    (number_of_courses, number_of_credits_attempted, 
     number_of_credits_failed, number_of_credits_earned, 
     cumulative_gpa, semester_gpa, course_pass_rate, 
     online_course_rate) = generate_valid_enrollment_data()

    data.append([
        student_id, semester, enrollment_type, degree_type, 
        intent_to_transfer_flag, pell_recipient, major, dept, 
        cumulative_gpa, semester_gpa, number_of_courses, 
        number_of_credits_attempted, number_of_credits_failed, 
        number_of_credits_earned, course_pass_rate, online_course_rate
    ])
...
columns = [
    "Student_ID", "Semester", "Course_Prefix", "Course_Number", 
    "Course_Type", "Pass/Fail_Flag", "Grade", 
    "Prerequisite_Course_Flag", "Online_Course_Flag", 
...
modalities = [
    'Online', 'Face to Face', 'Online/Synch', 
    'Hybrid - Online/iTV', 'Hybrid - F2F/Online', 
...
for student_id in student_ids:
    ...
    for _, semester_row in student_semesters.iterrows():
        ...
        for _ in range(number_of_courses):
            ...
            data.append([
                student_id, semester, course_prefix, course_number, 
                course_type, pass_fail_flag, grade, 
                prerequisite_flag, online_course_flag, 
                number_of_credits_attempted, number_of_credits_earned, 
                modality, weeks
            ])
