...
from student_success_tool.reporting.sections.metric_sections import register_metric_sections
...
def mock_card():
    ...
    card.training_data = pd.DataFrame({"feature": [1, 2], "sample_weight_1": [0.2, 0.8]})
...
def test_register_metric_sections_primary_metric(mock_card):
    ...
    assert "- Our primary metric for training was recall" in rendered["primary_metric_section"]
