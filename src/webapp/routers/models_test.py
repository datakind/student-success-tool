...
from ..test_helper import MODEL_OBJ
...
def test_read_inst_models():
    ...
    response = client.get("/institutions/345/models")
...
def test_read_inst_model():
    ...
    # Authorized.
    response = client.get("/institutions/345/models/10")
    assert response.status_code == 200
    assert response.json() == MODEL_OBJ
    # Unauthorized cases.
    with pytest.raises(HTTPException) as err:
        client.get("/institutions/345/models/10")
...
def test_read_inst_model_versions():
    ...
    # Authorized.
    response = client.get("/institutions/345/models/10/vers")
    assert response.status_code == 200
    assert response.json() == []
...
def test_read_inst_model_version():
    ...
    # Authorized.
    response = client.get("/institutions/345/models/10/vers/0")
    assert response.status_code == 200
    assert response.json() == MODEL_OBJ
...
def test_read_inst_model_outputs():
    ...
    # Authorized.
    response = client.get("/institutions/345/models/10/vers/0/output")
    assert response.status_code == 200
    assert response.json() == []
...
def test_read_inst_model_output():
    ...
    # Authorized.
    response = client.get("/institutions/345/models/10/vers/0/output/1")
    assert response.status_code == 200
    assert response.json() == {
        "m_id": 10,
        "vers_id": 0,
...
def test_train_new_model():
    ...
    response = client.post("/institutions/345/models/")
    assert response.status_code == 200
...
def test_retrain_model():
    ...
    response = client.post("/institutions/345/models/123/vers/")
    assert response.status_code == 200
