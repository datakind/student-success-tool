...
class TrainingConfig(pyd.BaseModel):
    ...
    primary_metric: t.Literal[
        "auc", "logloss", "mean_per_class_error", "rmse", "mae", "aucpr"
    ] = pyd.Field(
        default="logloss",
...
class InferenceConfig(pyd.BaseModel):
    ...
    @pyd.field_validator("background_data_sample", mode="after")
    @classmethod
    def check_bd_sample(cls, value: int) -> int:
        if value < 500 or value > 2000:
            raise pyd.ValidationError(
                "Background data must be at least 500 samples in production for GBM/DRF use cases as top 5/10 will be stable."
                " Greater than 2000 samples is not necessary as accuracy gains are VERY small and runtime cost grows linearly."
            )
        return value
