...
def make_labeled_dataset(
    ...
    *,
    ...
    years_to_bach_at_cohort_col: str = "first_year_to_bachelors_at_cohort_inst",
    max_years_to_graduation: int = 4,
    ...
    if exclude_pre_cohort_terms is True:
        df_features = shared.get_first_student_year_term_within_cohort(
        ...
        df_features = shared.get_first_student_year_term(
            df_eligible_student_terms,
            student_id_cols=student_id_cols,
            sort_cols=term_rank_col,
            include_cols=None,
    ...
    df_targets = compute_target_variable(
        df_eligible_student_terms,
        student_id_cols=student_id_cols,
        years_to_bach_at_cohort_col=years_to_bach_at_cohort_col,
    )
...
def compute_target_variable(  
    ...
    *,  
    ...
    Args:
        df
        student_id_cols
        years_to_bach_at_cohort_col
    """ 
    student_id_cols = utils.to_list(student_id_cols)  
    return (  
    df[student_id_cols + [years_to_bach_at_cohort_col]]  
    .drop_duplicates(subset=student_id_cols, ignore_index=True)  
    .assign(target=lambda df: (df[years_to_bach_at_cohort_col] > max_years_to_graduation) | df[years_to_bach_at_cohort_col].isna())  
    .fillna({"target": False})  
    .astype({"target": "bool"})  
